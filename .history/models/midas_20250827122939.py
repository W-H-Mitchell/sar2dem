...
def _initialize_decoder_from_scratch(model):
    ...
    # First apply the standard output conv initialization
    _random_init_output_conv(model)
    
    # Then reinitialize all other decoder layers
    if hasattr(model, 'scratch'):
        print("Reinitializing scratch (decoder) module...")
        
        def init_decoder_weights(m):
            # Skip if this module has children (we want to init leaf modules)
            if list(m.children()):
                return
