...
def denormalize_dem(pred_raw, gt_min, gt_max):
    """
    Normalize prediction to [0,1] using its own range, 
    then scale to ground truth range
    """
    # First normalize to [0,1] using the prediction's own min/max
    pred_min = np.min(pred_raw)
    pred_max = np.max(pred_raw)
    
    # Handle edge case where prediction is constant
