...
class Config:
    ...
    MODEL_PATHS = {
        "DPT": "./models/DPT-L1_ep200.pth",
        "pix2pixHD": "./models/pix2pixHD_ep200.pth",
...
def load_pix2pixhd(checkpoint_path, device='cuda'):
...
def load_im2height_model(checkpoint_path, device):
    ...
    # Missing model instantiation!
    model = Im2Height()  # Need to add this
    state_dict = torch.load(checkpoint_path, map_location=device)
    model.load_state_dict(state_dict, strict=False)
    model.to(device).eval()
    return model
