...
def load_dpt_model(checkpoint_path, device):
    ...
    # handle state dict from fabric/ddp training
...
def load_pix2pixhd_model(checkpoint_path, device):
    """load pix2pixHD generator"""
    # simplified GlobalGenerator implementation
    from torch.nn import Sequential, Conv2d, ConvTranspose2d, ReflectionPad2d
    from torch.nn import InstanceNorm2d, ReLU, Tanh
    ...
    class ResnetBlock(nn.Module):
        def __init__(self, dim):
            ...
            self.conv_block = Sequential(
                ReflectionPad2d(1),
                Conv2d(dim, dim, kernel_size=3, padding=0),
                InstanceNorm2d(dim),
                ReLU(True),
                ReflectionPad2d(1),
                Conv2d(dim, dim, kernel_size=3, padding=0),
    ...
    # initial convolution
    ngf = 64
    model.add_module('initial_pad', ReflectionPad2d(3))
    model.add_module('initial_conv', Conv2d(3, ngf, kernel_size=7, padding=0))
    model.add_module('initial_norm', InstanceNorm2d(ngf))
    model.add_module('initial_relu', ReLU(True))
    
    # downsample
    n_downsampling = 4
    for i in range(n_downsampling):
        mult = 2**i
        model.add_module(f'down{i}_conv', 
                        Conv2d(ngf * mult, ngf * mult * 2, 
                              kernel_size=3, stride=2, padding=1))
        model.add_module(f'down{i}_norm', InstanceNorm2d(ngf * mult * 2))
        model.add_module(f'down{i}_relu', ReLU(True))
    
    # resnet blocks
    mult = 2**n_downsampling
    for i in range(9):
        model.add_module(f'resnet_{i}', ResnetBlock(ngf * mult))
    
    # upsample
    for i in range(n_downsampling):
        mult = 2**(n_downsampling - i)
        model.add_module(f'up{i}_conv',
                        ConvTranspose2d(ngf * mult, int(ngf * mult / 2),
                                      kernel_size=3, stride=2,
                                      padding=1, output_padding=1))
        model.add_module(f'up{i}_norm', InstanceNorm2d(int(ngf * mult / 2)))
        model.add_module(f'up{i}_relu', ReLU(True))
    
    # output layer
    model.add_module('out_pad', ReflectionPad2d(3))
    model.add_module('out_conv', Conv2d(ngf, 1, kernel_size=7, padding=0))
    model.add_module('out_tanh', Tanh())
...
def load_im2height_model(checkpoint_path, device):
    """load im2height model (using same architecture as pix2pixhd for simplicity)"""
    # for im2height, we can use the same generator architecture
    model = load_pix2pixhd_model(checkpoint_path, device)
    return model
...
def denormalize_dem(pred_norm, gt_min, gt_max):
    """scale normalized predictions to ground truth range"""
    # assume model outputs are normalized to [0, 1] range
...
def main():
    ...
    # update model paths
    config.MODEL_PATHS = {}
    if args.dpt_path and os.path.exists(args.dpt_model):
        config.MODEL_PATHS["DPT"] = args.dpt_model
    if args.pix2pixhd_path and os.path.exists(args.pix2pixhd_model):
        config.MODEL_PATHS["pix2pixHD"] = args.pix2pixhd_model
    if args.im2height_path and os.path.exists(args.im2height_model):
