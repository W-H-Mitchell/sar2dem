#!/usr/bin/env python3
"""
SAR to DEM Prediction Script
Simplified version for reproducible research
"""
...
#------------------------------------------------------------------------------
# CONFIGURATION - User should modify these paths
class Config:
    # Model paths - update these to your downloaded model weights
    MODEL_PATHS = {
        "DPT": "./models/dpt_sar2dem.pth",
        "Pix2PixHD": "./models/pix2pixhd_sar2dem.pth",
        "Im2Height": "./models/im2height_sar2dem.pth"
    }
...
def load_pix2pixhd_model(checkpoint_path, device):
    ...
    from models.pix2pixhd_generator import GlobalGenerator
...
def load_im2height_model(checkpoint_path, device):
    ...
    # Im2Height uses a U-Net architecture
    from models.im2height_unet import UNet
...
def run_inference(model, sar_tensor, device, model_type="dpt"):
    ...
    with torch.no_grad():
        ...
        if model_type == "pix2pixhd":
            # Pix2PixHD outputs in [-1, 1] range
            output = model(sar_tensor)
            output = (output + 1) / 2  # Convert to [0, 1]
        else:
            # DPT and Im2Height
            output = model(sar_tensor)
